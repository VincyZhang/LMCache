...
def generate_test_data(
    ...
    allocator = AdHocMemoryAllocator(
        device=torch_device_type if (hasattr(torch, torch_device_type) and getattr(torch, torch_device_type).is_available()) else "cpu",
    )
...
def create_test_config(
    buffer_device: str = torch_device_type if (hasattr(torch, torch_device_type) and getattr(torch, torch_device_type).is_available()) else "cpu",
    backend: str = "GDS_MT" if (hasattr(torch, torch_device_type) and getattr(torch, torch_device_type).is_available()) else "POSIX",
) -> LMCacheEngineConfig:
