...
def test_per_layer_scalar_accessors_sglang_mha():
    ...
    k = [torch.randn(512, 8, 64, dtype=torch.bfloat16, device=torch_device_type) for _ in range(2)]
    v = [torch.randn(512, 8, 64, dtype=torch.bfloat16, device=torch_device_type) for _ in range(2)]
    kv_caches = [k, v]
...
def test_get_group_data_ptrs_sglang_mha_groups_k_before_v():
    ...
    k = [torch.randn(512, 8, 64, dtype=torch.bfloat16, device=torch_device_type) for _ in range(3)]
    v = [torch.randn(512, 8, 64, dtype=torch.bfloat16, device=torch_device_type) for _ in range(3)]
    kv_caches = [k, v]
    fmt = lmc_ops.EngineKVFormat.TWO_X_NL_X_NBBS_NH_HS
...
def test_get_group_data_ptrs_cross_layer_returns_single_base():
    ...
    big = torch.empty(32, 80, 2, 16, 8, 64, dtype=torch.bfloat16, device=torch_device_type)
...
def test_get_device_handles_every_kvcaches_shape():
    ...
    flat = [torch.empty(4, dtype=torch.bfloat16, device=torch_device_type) for _ in range(3)]
    assert get_device(flat) == flat[0].device

    k = [torch.empty(4, dtype=torch.bfloat16, device=torch_device_type) for _ in range(2)]
    v = [torch.empty(4, dtype=torch.bfloat16, device=torch_device_type) for _ in range(2)]
